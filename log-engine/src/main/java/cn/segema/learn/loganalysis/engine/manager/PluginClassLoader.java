	...
	public void addToClassLoader(final String baseDir, final FileFilter filter, boolean quiet) {
		...
		if (base != null && base.exists() && base.isDirectory()) {
			...
			if (files == null || files.length == 0) {
				if (!quiet) {
					logger.error("No files added to classloader from lib: " + baseDir + " (resolved as: "
			...
			if (!quiet) {
				logger.error("Can't find (or read) directory to add to classloader: " + baseDir + " (resolved as: "
						+ base.getAbsolutePath() + ").");
			}
		}
	}
